// comment.js — handles comments AND likes, stored in comments.json in GitHub
...
async function saveCommentsFile(data, sha) {
...
export async function handler(event) {
  ...
  const { data, sha } = await getCommentsFile();

  // Ensure structure exists for this post
  ...
  if (action === 'like') {
    data[postId].likes += 1;
    try {
      await saveCommentsFile(data, sha);
